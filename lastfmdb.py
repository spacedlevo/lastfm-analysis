...
def get_lastfm_scrobbles():
    responses = []
    page = 1
    api_url = "https://ws.audioscrobbler.com/2.0/?method=user.getRecentTracks&user={}&api_key={}&limit=200&extended=0&page={}&format=json&from={}".format(
        creds["user"], creds["api_key"], page, from_date
    )
...
def add_album(album, artist_id):
    ...
    if album_id is None:
        cur.execute(
            "INSERT INTO albums (mbid, name, artist_id) VALUES (?, ?, ?)",
            (album["mbid"], album["#text"], artist_id),
        )
        cur.execute(q, (artist_id, album["#text"]))
        album_id = cur.fetchone()
    return album_id[0]
...
def add_track(track, artist_id, album_id):
    ...
    if track_id is None:
        cur.execute(
            "INSERT INTO tracks (mbid, name, artist_id, album_id) VALUES (?, ?, ?, ?)",
            (track["mbid"], track["name"], artist_id, album_id),
        )
        cur.execute(q, (artist_id, track["name"]))
        track_id = cur.fetchone()
    return track_id[0]
...
def add_listen(track, track_id):
    cur.execute(
        "INSERT OR IGNORE INTO listens (timestamp, track_id) VALUES (?, ?)",
        (track["date"]["uts"], track_id),
    )
