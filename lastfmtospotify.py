...
con = sqlite3.connect("data/lastfmscrobbles.db")
q = """
...
# Prepare data
scrobbles["Date Played"] = pd.to_datetime(
    scrobbles["Date Played"].astype(int), unit="s"
)
# Convert UTC timestamps to GMT
gmt = pytz.timezone("Europe/London")
scrobbles["Date Played"] = (
    scrobbles["Date Played"].dt.tz_localize(pytz.utc).dt.tz_convert(gmt)
)
...
def play_counts(df):
    df["Play Count"] = df.groupby(["Track", "Artist"], as_index=False)[
        "Track ID"
    ].transform("count")
...
playlist = last_played(scrobbles, 30, True)
...
recent_popular = select_based_plays(recent_popular, 3, True)
